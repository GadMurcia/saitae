/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package net.delsas.saitae.beans;

import java.util.List;
import javax.ejb.Stateless;
import javax.persistence.EntityManager;
import javax.persistence.PersistenceContext;
import net.delsas.saitae.entities.Capacitaciones;

/**
 *
 * @author delsas
 */
@Stateless
public class CapacitacionesFacade extends AbstractFacade<Capacitaciones> implements CapacitacionesFacadeLocal {

    @PersistenceContext(unitName = "net.delsas_saitae-ejb_ejb_1.0-SNAPSHOTPU")
    private EntityManager em;

    @Override
    protected EntityManager getEntityManager() {
        return em;
    }

    public CapacitacionesFacade() {
        super(Capacitaciones.class);
    }

    @Override
    public List<Capacitaciones> findByIdPersona(Integer idpersona) {
        return em.createNamedQuery("Capacitaciones.findByIdMaestro")
                .setParameter("idMaestro", idpersona)
                .getResultList();
    }

}
